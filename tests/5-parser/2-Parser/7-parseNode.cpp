...
struct TestInclude {
	...
	static ieml::FilePath getFilePath(const ieml::FilePath&, ieml::FilePath&& newFilePath) {
		return newFilePath.concat(".ieml");
	}
};

TEST(parser, Parser_parseNode) {
	{
		ieml::String str{R"(null)"};
		ieml::Parser parser{str};
		auto node{parser.parseNode(2)};
	...
	}
	{
		...
		ASSERT_EQ(std::get<6>(node.data_).filePath_, ieml::FilePath{"test.ieml"});
