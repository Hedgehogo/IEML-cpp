...
namespace ieml {
	...
	inline void pasteIf(const String& str, String& result, String::const_iterator& resultEnd, T& findBegin) {
	...
	String preprocess(const String& str) {
		String::const_iterator resultEnd{str.cbegin()};
		String result{};
		while(resultEnd != str.end()) {
			auto generalFind{ctre::search<rePreprocess>(resultEnd, str.cend())};
			if(generalFind) {
				String::const_iterator findBegin{generalFind.begin()};
