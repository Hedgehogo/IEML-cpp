...
namespace ieml {
	...
	llint toIntFromBasis(String::const_iterator first, String::const_iterator last) {
		String::const_iterator start{first + ((first[0] == '-' || first[0] == '+') ? 3 : 2)};
		int basis{*(start - 1) + 1 - 'a'};
		llint number{toDigit(*start)};
		for(String::const_iterator iter = start + 1; iter < last && *iter != ' '; ++iter) {
			if(*iter == '_')
				continue;
	...
	llint toIntFromDecimal(String::const_iterator first, String::const_iterator last) {
		String::const_iterator start{first + ((first[0] == '-' || first[0] == '+') ? 1 : 0)};
