...
namespace ieml {
	...
	namespace detail {
		...
		static auto has_custom_file_path_helper(FileInclude_&&) -> decltype(FileInclude_::get_file_path(std::declval<FilePath>(), std::declval<FilePath>()), std::true_type{});
	...
	Option<BasicFileData<Char_>> BasicParser<Char_, FileInclude_>::parse_file(Size indent) {
		...
		if(auto find{match_and_move<re_file, Char_>(pos_, end(), mark_)}) {
			...
			return {BasicFileData<Char_>{BasicNode<Char_>{FileInclude_::include(loaded_anchor_keeper, loaded_file_path), mark}, loaded_file_path, loaded_anchor_keeper}};
		}
		return {};
	}
}